...
const USERS_STORAGE_KEY = 'masala_atelier_restaurant7_users_db';
const CURRENT_USER_KEY = 'masala_atelier_restaurant7_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt and pepper
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'masala_atelier_contemporary_gastronomy_pepper_2026');
...
const SEED_USERS = [
  {
    id: 'a7b8c9d0-e1f2-3456-789a-bcde01234567',
    name: 'Rohan Deshmukh',
    email: 'rohan.deshmukh@atelier.example',
  ...
  {
    id: 'b8c9d0e1-f2a3-4567-89ab-cde012345678',
    name: 'Sneha Reddy',
    email: 'sneha.reddy@example.com',
    salt: '2b3c4d5e6f7a8b9c',
    passwordHash: '',
    role: 'Gourmet Club Patron',
    createdAt: new Date().toISOString()
  },
  {
    id: 'c9d0e1f2-a3b4-5678-9abc-de0123456789',
    name: 'Vikram Sethi',
    email: 'vikram.sethi@example.com',
    salt: '5e6f7a8b9c0d1e2f',
    passwordHash: '',
    role: 'Tasting Flight Member',
    createdAt: new Date().toISOString()
  }
];
...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!storedUsersRaw) {
          ...
          for (const seed of SEED_USERS) {
            let initialPassword = 'Atelier@123';
            if (seed.email.includes('rohan')) initialPassword = 'Gastronomy@123';
            if (seed.email.includes('vikram')) initialPassword = 'Fusion@123';
        ...
        console.error('Failed to initialize auth state in restaurant-7:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getDemoUsers = () => {
    return [
      { role: 'Executive Chef & Founder', name: 'Rohan Deshmukh', email: 'rohan.deshmukh@atelier.example', pass: 'Gastronomy@123' },
      { role: 'Gourmet Club Patron', name: 'Sneha Reddy', email: 'sneha.reddy@example.com', pass: 'Atelier@123' },
      { role: 'Tasting Flight Member', name: 'Vikram Sethi', email: 'vikram.sethi@example.com', pass: 'Fusion@123' }
    ];
  };
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in restaurant-7');
  }
  return context;
};
