...
const USERS_STORAGE_KEY = 'ember_olive_restaurant2_users_db';
const CURRENT_USER_KEY = 'ember_olive_restaurant2_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256
async function hashPassword(password: string, salt: string): Promise<string> {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'ember_olive_secret_pepper_2026');
...
const SEED_USERS = [
  {
    ...
    salt: '8f92a10b4c7d6e5f',
    // Pre-computed SHA-256 for 'EmberPass123!' with salt '8f92a10b4c7d6e5f'
    passwordHash: '',
    role: 'VIP Member',
    createdAt: new Date().toISOString()
  },
  {
    id: 'f9876543-21ba-dcfe-0987-654321fedcba',
    name: 'Ananya Sharma',
    email: 'ananya@example.com',
    salt: '3d5e7f9a1b2c4e6d',
    passwordHash: '',
    role: 'Patron',
    createdAt: new Date().toISOString()
  }
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Initialize and seed simulated PostgreSQL database in localStorage if not present
  useEffect(() => {
