...
const USERS_STORAGE_KEY = 'rang_mahal_restaurant8_users_db';
const CURRENT_USER_KEY = 'rang_mahal_restaurant8_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'rang_mahal_rajasthani_secret_pepper_2026');
...
const SEED_USERS = [
  ...
  },
  {
    ...
    passwordHash: '',
    role: 'Heritage Club Member',
    createdAt: new Date().toISOString()
  },
  {
    id: 'c3d4e5f6-a7b8-9012-cdef-123456789012',
    name: 'Chef Bheem Singh',
    email: 'chef.bheem@rangmahal.example',
    salt: '4f5e6d7c8b9a0f1e',
    passwordHash: '',
    role: 'Executive Heritage Chef',
    createdAt: new Date().toISOString()
  }
];
...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!storedUsersRaw) {
          ...
          for (const seed of SEED_USERS) {
            let initialPassword = 'Marwar@123';
            if (seed.email.includes('sneha')) initialPassword = 'Palace@123';
            if (seed.email.includes('chef')) initialPassword = 'MasterChef@123';
        ...
        console.error('Failed to initialize auth state in restaurant-8:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!userRecord) {
        return { success: false, error: 'No royal patron account found with this email. Please register or use a demo account.' };
      }

      const inputHash = await hashPassword(password, userRecord.salt);
      if (inputHash !== userRecord.passwordHash) {
        return { success: false, error: 'Incorrect password. Please verify your royal credentials.' };
      }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      return { success: false, error: 'Failed to create your Rang Mahal account.' };
    }
  };
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in restaurant-8');
  }
  return context;
};
