...
const USERS_STORAGE_KEY = 'aidendrake_portfolio2_users_db';
const CURRENT_USER_KEY = 'aidendrake_portfolio2_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'aiden_drake_portfolio2_pepper_2026');
...
// Initial seed users (matching PostgreSQL seed data in portfolio_2_auth.sql)
const SEED_USERS = [
  {
    id: 'a2020202-1111-2222-3333-444444444444',
    name: 'Marcus Vance',
    email: 'marcus@vancedesign.com',
    salt: 'a2a2a2a2a2a2a2a2',
    passwordHash: '',
    role: 'Senior Product Lead',
    createdAt: new Date().toISOString()
  },
  {
    id: 'b2020202-2222-3333-4444-555555555555',
    name: 'Sarah Lin',
    email: 'sarah.lin@fintechinnovate.io',
    salt: 's2s2s2s2s2s2s2s2',
    passwordHash: '',
    role: 'Design Systems Director',
    createdAt: new Date().toISOString()
  },
  {
    id: 'c2020202-3333-4444-5555-666666666666',
    name: 'Aiden Drake',
    email: 'aiden@aidendrake.design',
    salt: 'p2p2p2p2p2p2p2p2',
    passwordHash: '',
...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!storedUsersRaw) {
          ...
          for (const seed of SEED_USERS) {
            let initialPassword = 'AidenDrake2026!';
            if (seed.email.includes('aiden@aidendrake')) initialPassword = 'AidenDrakePrincipal2026!';
        ...
        console.error('Failed to initialize auth state in portfolio-2:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!userRecord) {
        return { success: false, error: 'No client profile registered with this email. Please check your credentials or register.' };
      }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      const newUser = {
        id: window.crypto.randomUUID ? window.crypto.randomUUID() : 'ad_p2_usr_' + Date.now(),
        name: cleanName,
        email: cleanEmail,
        salt: salt,
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in portfolio-2');
  }
  return context;
};
