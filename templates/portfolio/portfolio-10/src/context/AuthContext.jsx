...
const USERS_STORAGE_KEY = 'sashagrey_portfolio10_users_db';
const CURRENT_USER_KEY = 'sashagrey_portfolio10_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'sasha_grey_editorial_luxury_pepper_2026');
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        console.error('Failed to initialize auth state in portfolio-10:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!userRecord) {
        return { success: false, error: 'No client profile registered with this email. Please create an account or check your credentials.' };
      }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      const newUser = {
        id: window.crypto.randomUUID ? window.crypto.randomUUID() : 'sg_usr_' + Date.now(),
        name: cleanName,
        email: cleanEmail,
        salt: salt,
      ...
      return { success: false, error: 'Failed to create your Sasha Grey client account.' };
    }
  };
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in portfolio-10');
  }
  return context;
};
