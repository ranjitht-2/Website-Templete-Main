...
const USERS_STORAGE_KEY = 'sashagrey_portfolio3_users_db';
const CURRENT_USER_KEY = 'sashagrey_portfolio3_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'sasha_grey_geometric_portfolio3_pepper_2026');
...
// Initial seed users (matching PostgreSQL seed data in portfolio_3_auth.sql)
const SEED_USERS = [
  {
    id: 'a3030303-1111-2222-3333-444444444444',
    name: 'Elena Rostova',
    email: 'elena@example.com',
    salt: 's3s3s3s3s3s3s3s3',
    passwordHash: '',
    role: 'Fashion Creative Director',
    createdAt: new Date().toISOString()
  },
  {
    id: 'b3030303-2222-3333-4444-555555555555',
    name: 'Dominic Vance',
    email: 'dominic@luxurybrand.com',
    salt: 'd3d3d3d3d3d3d3d3',
    passwordHash: '',
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        console.error('Failed to initialize auth state in portfolio-3:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!userRecord) {
        return { success: false, error: 'No client profile registered with this email. Please check your email or create a new account.' };
      }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      const newUser = {
        id: window.crypto.randomUUID ? window.crypto.randomUUID() : 'sg_p3_usr_' + Date.now(),
        name: cleanName,
        email: cleanEmail,
        salt: salt,
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in portfolio-3');
  }
  return context;
};
