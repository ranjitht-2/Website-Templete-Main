...
const USERS_STORAGE_KEY = 'blush_photography9_users_db';
const CURRENT_USER_KEY = 'blush_photography9_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'blush_lens_boutique_secret_pepper_2026');
...
const SEED_USERS = [
  {
    id: 'a9999999-1111-2222-3333-444444444444',
    name: 'Sarah Jenkins',
    email: 'sarah@example.com',
    salt: 'b9b9b9b9b9b9b9b9',
    passwordHash: '',
    role: 'Wedding Couple',
  ...
  },
  {
    ...
    salt: 'd9d9d9d9d9d9d9d9',
    passwordHash: '',
    role: 'Editorial Client',
    createdAt: new Date().toISOString()
  },
  {
    id: 'c9999999-3333-4444-5555-666666666666',
    name: 'Creative Director',
    email: 'director@blushlens.luxury',
    salt: 'c9c9c9c9c9c9c9c9',
    passwordHash: '',
...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!storedUsersRaw) {
          ...
          for (const seed of SEED_USERS) {
            let initialPassword = 'BlushPass123!';
            if (seed.email.includes('director')) initialPassword = 'BlushStudio2026!';
        ...
        console.error('Failed to initialize auth state in photography-9:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!userRecord) {
        return { success: false, error: 'No account found with this email. Please create a client account or verify your details.' };
      }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      return { success: false, error: 'Failed to create your Blush Lens account.' };
    }
  };
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in photography-9');
  }
  return context;
};
