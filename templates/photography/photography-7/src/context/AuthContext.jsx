...
const USERS_STORAGE_KEY = 'lume_photography7_users_db';
const CURRENT_USER_KEY = 'lume_photography7_current_user';

// Helper function to securely hash passwords using Web Crypto API SHA-256 with salt
async function hashPassword(password, salt) {
  const encoder = new TextEncoder();
  const data = encoder.encode(salt + password + 'lume_editorial_fashion_photography_secret_2026');
...
// Pre-seeded client accounts matching PostgreSQL table
const SEED_USERS = [
  {
    id: 'a7777777-1111-2222-3333-444444444444',
    name: 'Sarah Jenkins',
    email: 'sarah@velour.com',
    salt: 'l7l7l7l7l7l7l7l7',
    passwordHash: '',
    role: 'Creative Director',
    createdAt: new Date().toISOString()
  },
  {
    id: 'b7777777-2222-3333-4444-555555555555',
    name: 'Marcus Aurelius',
    email: 'marcus@zenith.com',
    salt: 'm7m7m7m7m7m7m7m7',
    passwordHash: '',
    role: 'Brand Producer',
    createdAt: new Date().toISOString()
  },
  {
    id: 'c7777777-3333-4444-5555-666666666666',
    name: 'Lume Studio Lead',
    email: 'director@lume-studio.com',
    salt: 'd7d7d7d7d7d7d7d7',
    passwordHash: '',
...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!storedUsersRaw) {
          ...
          for (const seed of SEED_USERS) {
            let initialPassword = 'LumePass123!';
            if (seed.email.includes('director')) initialPassword = 'MasterLume2026!';
        ...
        console.error('Failed to initialize auth state in photography-7:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!userRecord) {
        return { success: false, error: 'No client account found with this email. Please register or check your spelling.' };
      }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      return { success: false, error: 'Failed to create your Lume client account.' };
    }
  };
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error('useAuth must be used within an AuthProvider in photography-7');
  }
  return context;
};
